...
void print_unfinished_background_process(pid_node* head){
	if(head == NULL || head->process_id == -1){
		printf("No active background process detected\n");
		return;
	}
	...
	int count = 0;
	while(temp != NULL){
		if(temp->is_done == 0 && temp->process_id != -1){
			count++;
			printf("[============ Background Process with ID %ld is active ============]\n", (long)temp->process_id);
		}
...
void print_finished_background_process(pid_node* head){
	...
	while(temp != NULL){
		if(temp->is_done == 1 && temp->process_id != -1){
			printf("[============ Background Process with ID %ld finished and terminated ============]\n", (long)temp->process_id);
			printf("[Wall time: %f]\n", temp->wall_time);
...
int count_unfinished_node(pid_node* head){
	if(head == NULL){
		...
		if(head->is_done == 0){
			printf("Current node is list: %ld\n", (long)head->process_id);
			return 1 + count_unfinished_node(head->next);
		}
		else{
			return count_unfinished_node(head->next);
		}
	}
